/**
 * Test module for SSR Renderer - LocalState Inter-Field Dependencies.
 *
 * Coverage:
 ...
 * the localState initialization loop in renderLocalState does NOT
 ...
 * fix is applied to packages/server/src/renderer.ts renderLocalState().
 */
...
describe('renderToString - localState inter-field dependencies', () => {
  // ==================== Helper Functions ====================
  ...
  describe('field B references field A with literal initial value', () => {
    it('should resolve _gridMin from _bounds.min when _bounds is an object literal', async () => {
       ...
       * Then: The child text should render "0"
       *
       * Currently BROKEN: _bounds is not in ctx.locals when _gridMin is evaluated,
       * so expr:local returns undefined, expr:get on undefined returns undefined,
       * and the text renders as "" or "undefined".
       */
  ...
  describe('field B computes from two properties of field A', () => {
    it('should resolve _range as _bounds.max - _bounds.min', async () => {
       ...
       * When: renderToString is called
       * Then: The child text should render "190"
       *
       * Currently BROKEN: _bounds is undefined, so both get() calls return
       * undefined, and bin('-') produces NaN.
       */
      ...
      // Assert
      expect(result).toBe('<span>190</span>');
    });
  });
  ...
  describe('three-field chain A -> B -> C', () => {
    it('should resolve _tripled through chain _base -> _doubled -> _tripled', async () => {
       ...
       *   _base = 10 (literal)
       *   _doubled = _base * 2 (bin('*', local('_base'), lit(2)))
       *   _tripled = _doubled + _base (bin('+', local('_doubled'), local('_base')))
       * When: renderToString is called
       * Then: The child text should render "30" (10*2 + 10)
       *
      ...
      // Assert
      expect(result).toBe('<span>30</span>');
    });
  });
});
