...
describe('initClient', () => {
  ...
  describe('localStorage theme synchronization', () => {
    /**
     * BUG FIX: Theme state should be synchronized from localStorage on page load.
     *
     * Problem:
     * 1. User switches to light mode (localStorage saves 'light')
     * 2. Page reloads
     * 3. Anti-flash script updates DOM to light
     * 4. BUT state remains initial value 'dark'
     * 5. First button click: state 'dark' -> 'light' (DOM already light, no visual change)
     * 6. Second button click: state 'light' -> 'dark' (finally changes to dark)
     *
     * Solution:
     * On client initialization, read localStorage and sync state before any user interaction.
     */
    ...
    it('should sync state to light when localStorage has plain "light" value', async () => {
      // Arrange
      ...
      // Assert - setState should be called to sync theme to 'light'
      expect(mockSetState).toHaveBeenCalledWith('theme', 'light');
    });

    it('should sync state to light when localStorage has JSON serialized "light" value', async () => {
      // Arrange - JSON serialized value (e.g., from JSON.stringify)
      localStorage.setItem(THEME_STORAGE_KEY, '"light"');
      ...
      // Assert - setState should be called with parsed value 'light'
      expect(mockSetState).toHaveBeenCalledWith('theme', 'light');
    });

    it('should not call setState when localStorage is empty', async () => {
      // Arrange - no localStorage value
      ...
      // Assert - setState should NOT be called for theme
      // (it might be called for other reasons, so we check specifically for theme)
      const themeSetStateCalls = mockSetState.mock.calls.filter(
        (call: unknown[]) => call[0] === 'theme'
      );
      expect(themeSetStateCalls.length).toBe(0);
    });

    it('should not call setState when localStorage value matches current state', async () => {
      // Arrange - localStorage has 'dark', state is also 'dark'
      ...
      // Assert - setState should NOT be called for theme since values match
      const themeSetStateCalls = mockSetState.mock.calls.filter(
        (call: unknown[]) => call[0] === 'theme'
      );
      expect(themeSetStateCalls.length).toBe(0);
    ...
    it('should sync state to dark when localStorage has "dark" but current state is "light"', async () => {
      // Arrange
      localStorage.setItem(THEME_STORAGE_KEY, 'dark');
      ...
      // Assert - setState should be called to sync theme to 'dark'
      expect(mockSetState).toHaveBeenCalledWith('theme', 'dark');
    });

    it('should handle invalid JSON in localStorage gracefully', async () => {
      // Arrange - invalid JSON value
